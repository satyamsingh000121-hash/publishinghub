...
export function WorldMap({
  ...
  let currentTheme: "dark" | "light" = explicitTheme || "dark";
  try {
    const themeCtx = useAppTheme();
    if (!explicitTheme && themeCtx?.theme) {
      currentTheme = themeCtx.theme;
    }
  } catch {
    // If used outside ThemeProvider, fallback to explicit or dark
    if (explicitTheme) {
  ...
  return (
    <div className={`w-full aspect-[2/1] md:aspect-[2.2/1] relative font-sans overflow-hidden bg-transparent ${className}`}>
      <Image
        src={`data:image/svg+xml;utf8,${encodeURIComponent(svgMap)}`}
        className="h-full w-full [mask-image:linear-gradient(to_bottom,transparent,white_15%,white_85%,transparent)] pointer-events-none select-none object-cover"
        alt="world map"
        height={495}
        width={1056}
        draggable={false}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-center h-full">
                        <span
                          className={`font-medium px-2.5 py-0.5 rounded-md shadow-sm whitespace-nowrap ${labelClassName}`}
                        >
                          {dot.start.label}
                        </span>
                      </div>
